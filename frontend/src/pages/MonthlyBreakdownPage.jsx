...
/* ─── Month Card ─────────────────────────────────────────── */
function MonthCard({ period, isOpen, onToggle }) {
    ...
    <div style={{
      ...
      >
        ...
        <div style={{
          ...
          <span className="material-symbols-outlined" style={{ fontSize: 20, color: isOpen ? 'var(--primary)' : 'var(--secondary)' }}>
            calendar_month
          </span>
        </div>
        ...
        {/* Month label + stats */}
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: '0.9rem', fontWeight: 700, color: 'var(--on-surface)', lineHeight: 1.2 }}>
            {period.label}
          </div>
...
export default function MonthlyBreakdownPage() {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await apiClient.get('/analytics/historical?type=month');
      return data;
    },
    staleTime: 1000 * 60 * 5,
  });
      ...
      <div style={{ padding: '20px 16px', maxWidth: 640, margin: '0 auto' }}>
        ...
        {isLoading && (
          <div style={{ padding: '60px 0', textAlign: 'center' }}>
            ...
            <p style={{ color: 'var(--secondary)', fontSize: '0.8rem' }}>Loading monthly history…</p>
          </div>
        )}
        ...
        {!isLoading && !isError && data?.length === 0 && (
          <div className="empty-state">
            <span className="material-symbols-outlined empty-state__icon">calendar_month</span>
            <div className="empty-state__title">No monthly data yet</div>
        ...
        {!isLoading && !isError && data?.length > 0 && (
          <>
            <div style={{
              ...
              {data.length} month{data.length !== 1 ? 's' : ''} on record
            </div>
            {data.map((period) => (
              <MonthCard
